...
def get_R_at_angle(t, angle):
    ...
    # FIX: validity based on actual counts in that bin, not Gaussian value
    # Only mark invalid if the bin has zero counts (truly no data)
    if counts[idx] == 0:
        return np.nan
    ...
    # If Gaussian is truly negligible (< 1e-15), still return nan
    if p_gaus < 1e-15:
        return np.nan

    return p_data / p_gaus

# Print all R values so you can see them
print("R values at fixed angles:")
for angle in fixed_angles:
    print("  theta = " + str(angle) + " deg:")
    for t in thicknesses:
        R = get_R_at_angle(t, angle)
        print("    t = " + str(t) + "  ->  R = " + str(round(R, 3) if not np.isnan(R) else 'N/A'))
...
for angle, col in zip(fixed_angles, angle_colors):
    ...
    valid_t = [t for t, R in zip(thicknesses, R_vals) if not np.isnan(R)]
    valid_R = [R for R in R_vals if not np.isnan(R)]
    ...
    plt.savefig('phase2_block4_angle' + aname + '_v2.png', dpi=150)
    plt.show()
    print('Saved: phase2_block4_angle' + aname + '_v2.png')

# ═══════════════════════════════════════════════
# PLOT 4 — Combined log scale
# ═══════════════════════════════════════════════
...
plt.tight_layout()
plt.savefig('phase2_block4_combined_v2.png', dpi=150)
plt.show()
print('Saved: phase2_block4_combined_v2.png')

# ═══════════════════════════════════════════════
# PLOT 5 — Heatmap (relaxed validity)
# ═══════════════════════════════════════════════
...
log_matrix = np.where(np.isnan(R_matrix), np.nan,
                      np.log10(np.clip(R_matrix, 0.01, 1e6)))

fig, ax = plt.subplots(figsize=(8, 6))
im = ax.imshow(np.where(np.isnan(log_matrix), -1, log_matrix),
               aspect='auto', cmap='RdYlGn_r', vmin=0, vmax=5)
...
ax.set_title('Block 4 — Tail Ratio R  Heatmap  (log10 colour scale)\n'
             'Red = large tail excess  |  Green = near Gaussian',
...
for i in range(len(heatmap_angles)):
    for j in range(len(thicknesses)):
        ...
        if np.isnan(val):
            ...
            txt = str(round(val, 1)) if val < 1000 else '{:.1e}'.format(val)
            lv  = np.log10(max(val, 0.01))
            ax.text(j, i, txt, ha='center', va='center',
                    fontsize=11, fontweight='bold',
                    color='white' if lv > 2.5 else 'black')

cbar = plt.colorbar(im, ax=ax)
cbar.set_label('log10(R)', fontsize=11)
cbar.set_ticks([0, 1, 2, 3, 4, 5])
cbar.set_ticklabels(['1', '10', '100', '1000', '10⁴', '10⁵'])
plt.tight_layout()
plt.savefig('phase2_block4_heatmap_v2.png', dpi=150)
plt.show()
print('Saved: phase2_block4_heatmap_v2.png')
