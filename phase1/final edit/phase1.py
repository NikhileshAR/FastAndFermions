...
# ─────────────────────────────────────────
# Read and fit all histogram blocks
# ─────────────────────────────────────────
for line in lines:
    stripped = line.strip()
...
# ─────────────────────────────────────────
# Linear fit on sqrt(t) — linear regime only
# ─────────────────────────────────────────
lin_mask = t_s <= 0.25
slope, intercept, r, _, _ = linregress(sqrt_t[lin_mask], sig_s[lin_mask])
...
ax.plot(t_s, hl, 's--', color='darkorange', ms=4, lw=1.5,
...
# ═══════════════════════════════════════════════
# PLOT 2 — sigma vs sqrt(thickness)  + linear fit
# ═══════════════════════════════════════════════
fig, ax = plt.subplots(figsize=(9, 6))
...
# ═══════════════════════════════════════════════
# PLOT 3 — Highland comparison only
# ═══════════════════════════════════════════════
fig, ax = plt.subplots(figsize=(9, 6))
ax.errorbar(sqrt_t, sig_s, yerr=err_s, fmt='o', color='royalblue',
            ms=5, elinewidth=1, capsize=3, label='Measured sigma_core')
